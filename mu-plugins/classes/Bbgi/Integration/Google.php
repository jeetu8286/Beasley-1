...
class Google extends \Bbgi\Module {
	...
	static public function ga_enqueue_scripts() {
		...
		echo sprintf(
			'<script>window.bbgiAnalyticsConfig=JSON.parse(\'%s\');</script>',
			$jsonData,
		);

		$homeUrl = home_url($wp->request);
		$fileNameAddition = str_contains($homeUrl, '.beasley.test')
			|| (str_contains($homeUrl, '.bbgistage.com') && !str_contains($homeUrl, '-preprod.bbgistage.com'))? '-dev' : '';
		wp_enqueue_script(
