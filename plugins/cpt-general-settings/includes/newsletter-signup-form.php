...
class NewsletterSignupForm {
	...
	public function nsf_register_scripts() {
		...
		wp_register_script('nsf-script', GENERAL_SETTINGS_CPT_URL . 'assets/js/newsletter-signup-form'. $postfix .'.js', array('jquery'), '1.0.2');
		wp_localize_script( 
			'nsf-script',
			'nsf_ajax_object',
			array( 
	...
	public function nsf_function($attr) {
		...
		if ( !$nsf_output_hide ) {
			...
			$nsf_color						= $attr['nsf_color'] != '' ? sanitize_text_field($attr['nsf_color']) : (get_option('nsf_color') != '' ? get_option('nsf_color') : '#000000');
