...
function remove_cdn_files ( $post_id ) {
	...
	$attachment_metadata = $wpdb->get_results (
		$wpdb->prepare (
			"SELECT meta_key,meta_value FROM " . $wpdb->prefix . "postmeta WHERE post_id = '%d' AND (meta_key='_wp_attachment_metadata' OR meta_key='_wp_attached_file')",
			absint( $post_id )
		)
	);
	if ( $wpdb->num_rows > 0 ) {
...
function get_files_to_sync () {
	...
	if ( check_cdn () === false ) {
		return array (
			'response' => 'fail',
			'message'  => 'Error instantiating CDN session.',
			'upload'   => $objects_to_upload,
			'download' => $objects_to_download
		);
	}
	...
	if ( stripos ( $file_to_sync, 'http' ) === false ) {
		...
		if ( ! file_exists ( $file_to_sync ) ) {
			echo json_encode ( array (
					'response' => 'error',
					'message'  => 'Upload for "' . basename ( $file_to_sync ) . '" failed (SEF-001).'
				) );
			die();
		}
		...
		try {
			...
			echo json_encode ( array (
					'response' => 'error',
					'message'  => 'Upload for "' . basename ( $file_to_sync ) . '" failed. Exception: ' . $exc . ' (SEF-002).'
				) );
			die();
		}
...
function remove_existing_file () {
	...
	if ( verify_exists ( $cdn_url . '/' . $remote_file_name ) ) {
		...
		try {
			...
			echo json_encode ( array (
					'response' => 'error',
					'message'  => 'Upload for "' . basename ( $file_to_sync ) . '" failed (REF-002).'
				) );
			die();
		}
	}
		...
		if ( ! is_dir ( $the_file ) ) {
			if ( isset( $_SESSION['cdn']->api_settings->files_to_ignore ) ) {
				...
				if ( ! in_array ( $file_path['extension'], $ignore_files ) ) {
					...
					$local_files[ $i ++ ] = array (
						'fn' => trim ( str_replace ( $upload_dir['basedir'], '', $cur_local_file ), '/' ),
						'fs' => filesize ( $cur_local_file )
					);
				}
			} else {
