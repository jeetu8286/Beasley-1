	...
	<form method="post" action="">
		<div id="error_notifications">
			...
			if ( count ( $show_errors ) > 0 ) {
				foreach ( $show_errors as $cur_error ) {
					...
					<div id="setting-error-settings_updated" class="error settings-error">
						<p>
							<strong>Ruh-Roh!</strong><br /><?php echo isset( $cur_error ) ? $cur_error : 'Your settings are busted. Please verify and make sure you have the correct credentials.' ?>
						</p>
					</div>
				<?php
				}
				...
				<div id="setting-error-settings_updated" class="updated settings-error"<?php echo ( $num_files_to_sync == 0 ) ? ' style="display:none;"' : '' ?>>
					<p>
						<strong>Hey! You there!</strong><br />There <?php echo $show_file_count ?> that <?php echo $show_needs ?> synchronized with the CDN. Click the
						<em>Synchronize</em> button below to sync your files.</p>
				</div>
			<?php
			}
			...
			if ( ! is_writable ( RS_CDN_PATH ) || ! is_writable ( RS_CDN_PATH . 'object_cache' ) ) {
				...
				<div class="error">
					<p>
						<strong>Uh-Oh!</strong><br />Looks like the "object_cache" file is not writable. Please fix this to improve load time performance.
					</p>
				</div>
			<?php
			}
		...
		<table class="form-table"<?php echo ( $settings_error == true || count ( $show_errors ) > 0 ) ? ' style="display:none;"' : '' ?>>
			...
			<tr valign="top">
				...
				<td>
					...
					<textarea name="rs_cdn[files_to_ignore]" style="width:350px;height:150px;"><?php echo ( isset( $cdn_settings->files_to_ignore ) ) ? esc_html( $cdn_settings->files_to_ignore ) : ''; ?></textarea>
				</td>
			</tr>
			</tbody>
		</table>
		...
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td>
					<input name="rs_cdn[username]" type="text" value="<?php echo esc_attr( $cdn_settings->username ); ?>" class="regular-text" required="required" />
					<input name="rs_cdn_old[username]" type="hidden" value="<?php echo esc_attr( $cdn_settings->username ); ?>" />
				</td>
			</tr>
			<tr valign="top">
				<th scope="row">
				...
				<td>
					<input name="rs_cdn[apiKey]" type="text" value="<?php echo esc_attr( $cdn_settings->apiKey ); ?>" class="regular-text" required="required" />
					<input name="rs_cdn_old[apiKey]" type="hidden" value="<?php echo esc_attr( $cdn_settings->apiKey ); ?>" />
				</td>
			</tr>
			<tr valign="top">
				<th scope="row">
				...
				<td>
					<input name="rs_cdn[container]" type="text" value="<?php echo esc_attr( $cdn_settings->container ); ?>" class="regular-text" required="required" />
					<input name="rs_cdn_old[container]" type="hidden" value="<?php echo esc_attr( $cdn_settings->container ); ?>" />
				</td>
			</tr>
			<tr valign="top">
				<th scope="row">
				...
				<td>
					<input name="rs_cdn[custom_cname]" id="rs_cdn_custom_cname" type="text" value="<?php echo ( isset( $cdn_settings->custom_cname ) ) ?  esc_attr( $cdn_settings->custom_cname ) : ''; ?>" class="regular-text" />
				</td>
			</tr>
			<tr valign="top">
				<th scope="row">
			...
			<tr valign="top">
				...
				<td>
					...
					<input name="rs_cdn_old[region]" type="hidden" value="<?php echo esc_attr( $cdn_settings->region ); ?>" />
			...
			<tr valign="top">
				...
				<td>
					<input name="rs_cdn[url]" type="text" value="<?php echo esc_url( $cdn_settings->url ); ?>" class="regular-text" required="required" readonly="readonly" />
				</td>
			</tr>
			<tr valign="top">
				<th scope="row"></th>
				<td>
					<p><input class="button-primary" class="left" type="submit" name="save_cdn_settings" value="Save" />&nbsp;
					</p>
				</td>
			</tr>
			</tbody>
