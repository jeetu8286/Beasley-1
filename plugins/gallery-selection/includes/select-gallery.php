...
class ExistingGallerySelection {
	 ...
	 */
	public static function gallery_post_query_conflict(){
		...
		if ($gallery_data['new_data']) {
			$html .= '<ul class="select-gallery-ul">';

			foreach ($gallery_data['new_data'] as $post_data) {
				...
				$html .= '
					<li class="select-exist-gallery-li" gallery-id="'.$post_data->ID.'" slug-name="'.$post_data->post_name.'" onclick=" jQuery(\'.select-gallery-ul li\').removeClass(' . $jqueryEventSelectedClass .'); jQuery(\'.select-gallery-ul li\').css(\'box-shadow\', \'0 1px 2px 0 rgba(0, 0, 0, 0.2), 0 1px 5px 0 rgba(0, 0, 0, 0.19)\'); jQuery(this).addClass(' . $jqueryEventSelectedClass .'); " >
						<div style="width: 200px; height: 150px; display: flex;">
							<img
								class="img-attachment"
						...
						<div class="desc-lower-container" style="padding-bottom:10px;"> 
							<div class="desc-lower-title">Date:</div> <div> '.self::get_modified_gallery_date($post_data).'</div>
						</div>
					</li>';
				}
	...
	public static function gallery_print_media_templates() {
			...
			<div class="selectgallery__preview">
				<?php $html = self::gallery_post_query_conflict(); echo $html; ?>	
			</div>
			</script><?php
	}
