...
if ( ! function_exists( 'ee_the_lazy_background' ) ) :
	function ee_the_lazy_background( $image_id, $aspect = false ) {
		if ( ! $image_id ) {
			return;
		}

		if ( ee_is_jacapps() ) {
			printf( ' style="background-image: url(%s)"', bbgi_get_image_url( $image_id, 800, 500 ) );
			return;
		}
		...
		printf(
			' data-lazy-background="%s" data-width="%s" data-height="%s" data-aspect="%s"',
			esc_attr( $img[0] ),
			esc_attr( $img[1] ),
			esc_attr( $img[2] ),
			! empty( $aspect ) ? esc_attr( $aspect ) : 16 / 10
...
if ( ! function_exists( 'ee_the_lazy_background_thumbnail' ) ) :
	function ee_the_lazy_background_thumbnail( $post = null ) {
		$post = get_post( $post );

		$thumbnail_id = get_post_thumbnail_id( $post );
		$thumbnail_id = apply_filters( 'ee_post_thumbnail_id', $thumbnail_id, $post );

		ee_the_lazy_background( $thumbnail_id );
	}
endif;
