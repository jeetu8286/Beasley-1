...
if ( ! function_exists( 'ee_get_post_by_omny_audio' ) ) :
	function ee_get_post_by_omny_audio( $audio ) {
		...
		if ( $post_id === false ) {
			...
			$post_id = $wpdb->get_var( "SELECT post_id FROM {$wpdb->postmeta} WHERE (meta_key = 'omny-audio-url' OR meta_key='enclosure' OR meta_key = 'gmr-podcast-audio') AND meta_value LIKE '{$audio}%'" );
