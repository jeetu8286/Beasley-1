	...
	<section class="list-grid-section card">
		<?php
			foreach ($display_ca_archive_posts as $key=>$archive_ca_post) {
				...
				<?php if($key == 2) { ?>
					<article class="in-grid-ad -tablet-in-ad">
				...
				<?php } ?>
				<?php if($key == 3) { ?>
					<article class="in-grid-ad -desktop-in-ad">
						<div class="ad -footer -centered">
							<?php do_action( 'dfp_tag', 'bottom-leaderboard', false, array( array( 'pos', 2 ) ) ); ?>
						</div>
					</article>
				...
				<article id="<?php echo $key; ?>" <?php if($cap_is_sponsored) { echo 'class="bg-red bg-red-thumbnail"'; }?>>
					<?php
